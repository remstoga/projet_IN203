...
void simulation(bool affiche)
{
    ...
    contexte.taux_population = (size - 1) * 20'000;
    épidémie::Grille grille{contexte.taux_population};
    std::size_t jours_écoulés = 0;

    std::size_t taux_population_local = contexte.taux_population / (size - 1);
    ...
    if (rank != 0)
    {
        ...
        population.reserve(taux_population_local);

        auto [largeur_grille, hauteur_grille] = grille.dimension();
        // L'agent pathogène n'évolue pas et reste donc constant...
        épidémie::AgentPathogène agent(graine_aléatoire++);
        // Initialisation de la population initiale :
        graine_aléatoire += (rank - 1) * (contexte.taux_population / (size - 1));
        for (std::size_t i = 0; i < taux_population_local; ++i)
        {
        ...
        int nombre_immunisés_grippe = (taux_population_local * 23) / 100;
        ...
        while (!quitting)
        {
            ...
            if (jours_écoulés % 365 == 0) // Si le premier Octobre (début de l'année pour l'épidémie ;-) )
            {
                ...
                for (int ipersonne = nombre_immunisés_grippe; ipersonne < int(taux_population_local); ++ipersonne)
                {
            ...
            if (jours_écoulés % 365 == std::size_t(jour_apparition_grippe))
            {
                ...
                {
                    std::cout << "rank : " << rank << " ipersonne : " << ipersonne << std::endl;
                    population[ipersonne].estContaminé(grippe);
                }
            }

            int recv_reduce_buff[largeur_grille * hauteur_grille * 3];
            int send_reduce_buff[largeur_grille * hauteur_grille * 3];
            màjStatistique(grille, population);

            place_in_buffer(grille.getStatistiques(), send_reduce_buff);

            MPI_Allreduce(send_reduce_buff, recv_reduce_buff, largeur_grille * hauteur_grille * 3, MPI_INT, MPI_SUM, new_comm);

            recv_from_buffer(grille.getStatistiques(), recv_reduce_buff);
            ...
            if (rank == 1)
            {
                ...
                MPI_Status status;
                MPI_Iprobe(0, 0, MPI_COMM_WORLD, &can_receive_message, &status);

                if (can_receive_message)
                {
                    int recv_signal;
                    MPI_Recv(&recv_signal, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
    ...
    if (rank == 0)
    {
        ...
        while (!quitting)
        {
            ...
            int send_signal;
            MPI_Send(&send_signal, 1, MPI_INT, 1, 0, MPI_COMM_WORLD);
