// simulation_async_affiche_mpi.cpp (peut-être renommé)
...
void simulation(bool affiche)
{
    ...
    if (rank == 0)
    {
        ...
        while (!quitting)
        {
            std::chrono::steady_clock::time_point t_debut_boucle = std::chrono::steady_clock::now();
            ...
            jours_écoulés++;
            std::chrono::steady_clock::time_point t_fin_boucle = std::chrono::steady_clock::now();
            if (affiche)
